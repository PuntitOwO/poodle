...
func _connect_signals() -> void:
    ...
    for child_idx in get_child_count() - 1:
        ...
        child.connect(&"animation_finished", next_child.play.bind(_durations[child_idx]), CONNECT_ONE_SHOT)
...
func _disconnect_signals() -> void:
    ...
    for child_idx in get_child_count() - 1:
        var child = get_child(child_idx)
        var next_child = get_child(child_idx + 1)
        if child.is_connected(&"animation_finished", next_child.play.bind(_durations[child_idx])):
            child.disconnect(&"animation_finished", next_child.play.bind(_durations[child_idx]))
